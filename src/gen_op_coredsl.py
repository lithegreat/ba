    ...
    with open(output_filepath, 'w') as f:
        ...
        for index, row in df.iterrows():
            ...
            if 'io_2_element_width' in row and pd.notna(row['io_2_element_width']):
                ...
                rs2_width = -1  # Default value when io_2_element_width does not exist or is NaN

            f.write(f'        {operation_name} ' + '{\n')
            f.write('            operands: {\n')
            f.write(f'                unsigned<{rd_width}> rd [[reg_type=u{oo_1_element_width}]] [[out]];\n')
            f.write(f'                unsigned<{rs1_width}> rs1 [[reg_type=u{io_1_element_width}]] [[in]];\n')
            if rs2_width != -1:
                f.write(f'                unsigned<{rs2_width}> rs2 [[reg_type=u{io_2_element_width}]] [[in]];\n')
            else:
                f.write('                // rs2 operand not specified in input\n')
            f.write('            }\n')
            f.write('            encoding: auto;\n')
            f.write(f'            assembly: {{"OpenASIP_{filename}.{operation_name}", "{{name(rd)}}, {{name(rs1)}}, {{name(rs2)}}"}};\n')
            f.write('            behavior: {};\n')
            f.write('        }\n')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate instruction set from Excel file")
    parser.add_argument(
        "--filename",
        type=str,
        default="base",
    ...
    input_filepath = f'Operations/{filename}.xlsx'  # Replace with your input Excel file path
